...
struct LocalSearchRowWeightingComponent
{
    ...
    optimizationtools::IndexedBinaryHeap<std::pair<Penalty, double>> scores_in;
        ...
        const LocalSearchRowWeightingParameters& parameters)
{
    ...
    for (ComponentId component_id = 0;
        ...
        components[component_id].scores_in
            = optimizationtools::IndexedBinaryHeap<std::pair<Penalty, double>>(component.sets.size());
        for (SetPos pos = 0;
    ...
    for (output.number_of_iterations = 0;
        ...
        if (output.number_of_iterations % instance.number_of_sets() == 0) {
        ...
        for (SetId set_id_1: instance.element(element_id).sets) {
            ...
            // Look at non-neighbor sets.
            for (SetPos pos = 0; pos < 3; ++pos) {
            ...
            for (SetId set_id_2: set_neighbors[set_id_1]) {
